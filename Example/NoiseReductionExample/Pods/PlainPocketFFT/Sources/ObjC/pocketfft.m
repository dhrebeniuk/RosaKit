...
void copy_and_norm(double *c, double *p1, size_t n, double fct)
{
if (p1!=c)
  {
  if (fct!=1.)
...
int rfftp_forward(rfftp_plan plan, double c[], double fct)
{
...
static int pass_all(cfftp_plan plan, cmplx c[], double fct,
const int sign)
{
if (plan->length==1) return 0;
...
int cfftp_forward(cfftp_plan plan, double c[], double fct)
...
int rfftblue_forward(fftblue_plan plan, double c[], double fct)
{
    size_t n=plan->n;
    ...
    if (fftblue_fft(plan,tmp,-1,fct)!=0) { DEALLOC(tmp); return -1; }
    c[0] = tmp[0];
...
int execute_real_forward(const double *a1, double *resultMatrix, int cols, int rows, double fct) {
    ...
    int ndim = 2;//PyArray_NDIM(data);
    const npy_intp odim[] = {cols, rows};//PyArray_DIMS(data); -> .m.shape (65, 256)
